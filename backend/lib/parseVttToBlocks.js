...
function findAndParseVttForVideo(videoPath, appDir = null) {
    ...
    // Strip counter suffix for matching: "video_(1)" → "video"
    const baseNameNoCounter = baseName.replace(/_?\(\d+\)$/, '');
    
    console.log(`[VTT-PARSER] Searching VTT for video: ${baseName} (stripped: ${baseNameNoCounter})`);
    ...
    if (appDir) {
        ...
        searchDirs.push(path.join(appDir, 'downloads'));
    }
    ...
    if (parentDir !== dir) {
        ...
        searchDirs.push(path.join(parentDir, 'assets'));
        searchDirs.push(path.join(parentDir, 'downloads'));
    }
    ...
    for (const searchDir of uniqueDirs) {
        ...
        try {
            ...
            if (vttFiles.length > 0) {
                console.log(`[VTT-PARSER] Found ${vttFiles.length} VTT file(s) in ${searchDir}: ${vttFiles.slice(0, 5).join(', ')}`);
            }
            
            // Try exact match first (baseName.vtt or baseName.*.vtt)
            let vttFile = vttFiles.find(f => f.startsWith(baseName));
            
            // Try with counter-stripped base name
            if (!vttFile && baseNameNoCounter !== baseName) {
                vttFile = vttFiles.find(f => f.startsWith(baseNameNoCounter));
            }
            
            // Try with any language code pattern (e.g. video.en.vtt, video.id.vtt)
            if (!vttFile) {
                vttFile = vttFiles.find(f =>
                    (f.startsWith(baseName) || f.startsWith(baseNameNoCounter)) &&
            ...
            if (!vttFile) {
                const normalizedBase = baseNameNoCounter.toLowerCase().replace(/[^a-z0-9]/g, '');
                if (normalizedBase.length >= 5) { // Only fuzzy match if name is long enough
                    vttFile = vttFiles.find(f => {
            ...
            // Last resort: if only one VTT file exists in the directory, use it
            if (!vttFile && vttFiles.length === 1) {
                console.log(`[VTT-PARSER] Using only VTT file in directory: ${vttFiles[0]}`);
                vttFile = vttFiles[0];
            }
            ...
            if (vttFile) {
                ...
                if (blocks && blocks.length > 0) {
                    return { blocks, vttPath };
                } else {
                    console.warn(`[VTT-PARSER] VTT file found but contains no blocks: ${vttPath}`);
                }
            }
        } catch (err) {
    ...
    console.warn(`[VTT-PARSER] ✗ No VTT file found for: ${baseName} (searched ${uniqueDirs.length} dirs)`);
    return null;
}
...
function findAndParseWhisperForVideo(videoPath, appDir = null, options = {}) {
    ...
    console.log(`[WHISPER-PARSER] Searching transcript for: ${baseName} (stripped: ${baseNameNoCounter})`);
    ...
    for (const searchDir of uniqueDirs) {
        ...
        try {
            ...
            const searchBase = baseName.substring(0, Math.min(baseName.length, 30));
            const searchBaseNoCounter = baseNameNoCounter.substring(0, Math.min(baseNameNoCounter.length, 30));
            
            const jsonFile = files.find(f =>
                f.endsWith('_transcript.json') &&
                (f.startsWith(searchBase) || f.startsWith(searchBaseNoCounter))
            ) || files.find(f =>
                f.endsWith('.json') &&
                (f.toLowerCase().includes(baseName.toLowerCase().substring(0, 20)) ||
                 f.toLowerCase().includes(baseNameNoCounter.toLowerCase().substring(0, 20)))
            );

            if (jsonFile) {
                const jsonPath = path.join(searchDir, jsonFile);
                console.log(`[WHISPER-PARSER] ✓ Found transcript: ${jsonPath}`);
    ...
    console.warn(`[WHISPER-PARSER] ✗ No transcript found for: ${baseName}`);
    return null;
}
