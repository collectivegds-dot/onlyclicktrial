...
// Intercepts console.log/warn/error and broadcasts to all
// connected WebSocket clients. This makes terminal logs visible
// in the frontend "Live Web-Worker Stream" panel.
// ============================================================
let wss = null; // Initialized after server starts

function broadcastLog(level, args) {
    if (!wss || wss.clients.size === 0) return;
    const msg = args.map(a => typeof a === 'string' ? a : JSON.stringify(a)).join(' ');
    const payload = JSON.stringify({ type: 'LOG', level, msg, time: Date.now() });
...
// Intercept console methods — keep originals for terminal output
...
// ============================================================
// License check removed - ONLYCLICK is fully unlocked
    ...
    for (const strategyIndex of strategiesToTry) {
        ...
        try {
            const result = await new Promise((resolve, reject) => {
                ...
                // 10-minute hard timeout per subprocess
                const PROCESS_TIMEOUT_MS = 10 * 60 * 1000;
                const processTimer = setTimeout(() => {
                    if (!settled) {
                        settled = true;
                        console.warn(`[YTDLP-DL] Strategy ${strategy.name} timed out after 10 minutes — killing`);
                        try { subprocess.kill('SIGTERM'); } catch (_) { /* ignore */ }
                        reject({ code: -1, stderr: stderrBuffer + '\nProcess timed out after 10 minutes', is403: false, isFormatError: false, strategyUsed: strategy.name });
                    }
                }, PROCESS_TIMEOUT_MS);

                // Progress stall detection: kill if no output for 120s
                const STALL_TIMEOUT_MS = 120 * 1000;
                let stallTimer = setTimeout(() => {
                    if (!settled) {
                        settled = true;
                        console.warn(`[YTDLP-DL] Strategy ${strategy.name} stalled (no progress for 120s) — killing`);
                        try { subprocess.kill('SIGTERM'); } catch (_) { /* ignore */ }
                        reject({ code: -1, stderr: stderrBuffer + '\nProcess stalled (no output for 120s)', is403: false, isFormatError: false, strategyUsed: strategy.name });
                    }
                }, STALL_TIMEOUT_MS);
...
const mainDbBridge = new DbBridge(DB_FILE);
const readDb = async () => await mainDbBridge.read();
const writeDb = async (data) => await mainDbBridge.write(data);

// --- ASSETS DB SETUP ---
const ASSETS_FILE = path.join(DB_DIR, 'assets.json');
const assetsDbBridge = new DbBridge(ASSETS_FILE);
const readAssets = async () => await assetsDbBridge.read();
const writeAssets = async (data) => await assetsDbBridge.write(data);

// --- LOGOS DB SETUP ---
const LOGOS_FILE = path.join(DB_DIR, 'logos.json');
const logosDbBridge = new DbBridge(LOGOS_FILE);
const readLogos = async () => await logosDbBridge.read();
const writeLogos = async (data) => await logosDbBridge.write(data);

// --- PROMPTS DB SETUP ---
const PROMPTS_FILE = path.join(DB_DIR, 'prompts.json');
const promptsDbBridge = new DbBridge(PROMPTS_FILE);

const DEFAULT_PROMPTS = [
    {
        id: 'default-viral-id',
        name: 'Viral Indonesia',
        instruction: 'Cari momen paling viral dan relatable untuk audiens Indonesia. Fokus pada hook yang kuat di 3 detik pertama, humor receh, atau momen yang bikin orang comment "RELATABLE BANGET". Prioritaskan momen dengan ekspresi wajah kuat atau reaksi yang memancing emosi.',
        isDefault: true,
        icon: '🔥',
    ...
    },
    {
        ...
        instruction: 'Ekstrak insight dan wisdom terbaik dari podcast/interview ini. Cari momen dimana narasumber memberikan advice yang actionable, cerita personal yang menyentuh, atau hot take yang kontroversial. Setiap klip harus punya alur: HOOK → KONTEKS → PUNCHLINE.',
        isDefault: true,
        icon: '🎙️',
        createdAt: '2026-03-16T00:00:00.000Z',
    },
    {
        id: 'default-edukasi',
        name: 'Edukasi & Tips',
        instruction: 'Temukan momen edukatif yang bisa jadi konten "Save untuk nanti". Fokus pada tips praktis, tutorial singkat, fakta mengejutkan, atau penjelasan yang simpel tapi powerful. Hindari bagian yang terlalu teknis atau membosankan.',
        isDefault: true,
        icon: '📚',
        createdAt: '2026-03-16T00:00:00.000Z',
    },
    {
        id: 'default-drama',
        name: 'Drama & Kontroversial',
        instruction: 'Cari momen paling kontroversial, debat panas, atau statement yang bikin orang setuju/tidak setuju. Fokus pada hot takes, pendapat berlawanan, momen konfrontasi, atau pernyataan yang memicu diskusi di kolom komentar.',
        isDefault: true,
        icon: '🔥',
        createdAt: '2026-03-16T00:00:00.000Z',
    },
    {
        ...
        instruction: 'Ekstrak momen storytelling terbaik — cerita personal, pengalaman hidup, momen haru, atau cerita sukses/gagal yang inspiring. Prioritaskan alur cerita yang lengkap: setup → tension → resolusi. Klip harus bisa berdiri sendiri tanpa konteks video penuh.',
        isDefault: true,
        icon: '🎭',
        createdAt: '2026-03-16T00:00:00.000Z',
    },
...
const readPrompts = async () => {
    const userPrompts = await promptsDbBridge.read();
    return [...DEFAULT_PROMPTS, ...userPrompts.filter(p => !p.isDefault)];
};

const writePrompts = async (data) => {
    const userOnly = data.filter(p => !p.isDefault);
    await promptsDbBridge.write(userOnly);
};
// --------------------------------
...
// Serve visual editor (FreeCut embed) from editor/dist
const editorDistPath = path.join(__dirname, '..', 'editor', 'dist');
if (fs.existsSync(editorDistPath)) {
    // Editor needs COOP/COEP headers for SharedArrayBuffer support
    const editorStaticConfig = {
    ...
    app.use('/editor', express.static(editorDistPath, editorStaticConfig));
    console.log('[EDITOR] Serving visual editor from:', editorDistPath);
}
...
app.get('/api/rss', async (req, res) => {
    try {
        // Use static ONLYCLICK Medium feed
        const feedUrl = 'https://medium.com/feed/@onlyclick';
        ...
        // Process items
        const sourceName = feed.title || feed.creator || "ONLYCLICK";
        const allItems = [];
                    ...
                    await new Promise((resolve, reject) => {
                        const followRedirect = (url, redirectCount = 0) => {
                            ...
                            const req = protocol.get(url, {
                                headers: { 'User-Agent': 'ONLYCLICK/1.6' },
                                timeout: 60000
                            }, (response) => {
...
app.post('/api/youtube/download-stream', async (req, res) => {
    ...
    try {
        ...
        const subprocess = ytdlpSpawn(url, [
            '-f', 'bv*[height<=1080]+ba/b[height<=1080]/bestvideo[height<=1080]+bestaudio/best',
...
app.post('/api/youtube/download', async (req, res) => {
    ...
    switch (format_id) {
        case 'highest':
            formatArg = 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio[ext=m4a]/bestvideo+bestaudio/bv*+ba/b';
            break;
        case 'quality_1080':
            formatArg = 'bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio/bv*[height<=1080]+ba/b[height<=1080]/b';
            break;
        case 'quality_720':
            formatArg = 'bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=720]+bestaudio[ext=m4a]/bestvideo[height<=720]+bestaudio/bv*[height<=720]+ba/b[height<=720]/b';
            break;
        case 'quality_480':
            formatArg = 'bestvideo[height<=480][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=480]+bestaudio[ext=m4a]/bestvideo[height<=480]+bestaudio/bv*[height<=480]+ba/b[height<=480]/b';
            break;
        default:
            formatArg = format_id
                ? `${format_id}+bestaudio[ext=m4a]/${format_id}+bestaudio/bv*+ba/b`
                : 'bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio/bv*[height<=1080]+ba/b[height<=1080]/b';
            break;
    }
    ...
    ytdlpDownloadWithRetry(
        ...
        // Check for VTT caption files alongside the downloaded video
        const downloadDir = path.dirname(outputPath);
        const baseName = path.basename(outputPath, path.extname(outputPath));
        let hasCaptions = false;
        try {
            const dirFiles = fs.readdirSync(downloadDir);
            hasCaptions = dirFiles.some(f => f.startsWith(baseName) && f.endsWith('.vtt'));
            if (!hasCaptions) {
                console.log(`[DL-LOCAL] No VTT captions found for ${baseName}`);
                activeDownloads[downloadId].logs.push('No YouTube captions found for this video.');
            } else {
                console.log(`[DL-LOCAL] VTT captions found for ${baseName}`);
            }
        ...
        activeDownloads[downloadId].hasCaptions = hasCaptions;
...
app.post('/api/video/process', async (req, res) => {
    ...
    (async () => {
        if (process.env.VERCEL) {
            console.log(`[VERCEL] Offloading job ${jobId} to GitHub Actions...`);
            if (octokit && process.env.GITHUB_REPO) {
                ...
                try {
                    await octokit.request('POST /repos/{owner}/{repo}/dispatches', {
                        ...
                        client_payload: {
                            jobId,
                            videoUrl: config.source_url || relativeVideoPath,
                            config: JSON.stringify(config)
                        }
                    });
                    console.log(`[GITHUB] Successfully dispatched job ${jobId}`);
                } catch (err) {
                    console.error(`[GITHUB] Error dispatching job ${jobId}:`, err.message);
                }
            } else {
                console.error('[GITHUB] Missing GITHUB_TOKEN or GITHUB_REPO. Cannot offload rendering.');
            }
            return;
        }
        ...
        try {
            ...
            const processClip = async (clip) => {
                ...
                for (let i = 0; i < clip.timelines.length; i++) {
                    ...
                    const unbufferedStartSec = parseTime(clip.timelines[i].start);
                    tempRawClips.push({ path: tempRawPath, duration: durationSec, originalStart: startSec, unbufferedStart: unbufferedStartSec });
                ...
                for (let i = 0; i < tempRawClips.length; i++) {
                    ...
                    // 2. ASS Subtitles (styleSettings.subtitleEnabled — same system as reprocess-single)
                    if (styleSettings && styleSettings.subtitleEnabled) {
                        try {
                            ...
                            // Prefer Whisper word-level, fallback to VTT
                            ...
                            if (captionSource && captionSource.blocks && captionSource.blocks.length > 0) {
                                const segmentOriginalStart = tempClip.originalStart ?? 0;
                                const segmentUnbufferedStart = tempClip.unbufferedStart ?? segmentOriginalStart;
                                ...
                                const relevantBlocks = allTimedBlocks.filter(b =>
                                    b.endTime > segmentOriginalStart && b.startTime < segmentOriginalEnd
                                );
                                ...
                                if (relevantBlocks.length > 0) {
                                    ...
                                    let offsetBlocks = relevantBlocks.map(block => ({
                                        ...block,
                                    ...
                                    const assPath = path.join(clipsDir, `sub_${clip.clip_id}_${i}.ass`);
                                    ...
                                    lastLink = '[v_sub]';
                                    console.log(`[PHASE2-SUB] ASS subtitle added: ${localBlocks.length} blocks (${captionSourceType})`);
                                }
                            } else {
                                console.warn('[PHASE2-SUB] No caption source found for subtitle rendering');
                            }
                        } catch (subErr) {
                            console.error('[PHASE2-SUB] Subtitle rendering error:', subErr.message, '— skipping subtitles');
                        }
                    }
...
app.get('/api/job/:id', async (req, res) => {
    ...
    // 2. If in Vercel/Production, check Supabase
    if (process.env.VERCEL || process.env.NODE_ENV === 'production') {
        ...
        try {
            ...
            console.error(`[DB] Job status error for ${jobId}:`, err.message);
        }
    }

    // Fallback: Check if it exists in DB (completed)
    const history = readDb();
    const found = history.find(p => p.id === jobId);
    if (found) {
...
app.patch('/api/history/:projectId/clips/:clipIdx', async (req, res) => {
...
app.post('/api/video/reprocess-single', async (req, res) => {
                    ...
                    if (styleSettings?.subtitleEnabled) {
                        ...
                        if (captionSource && captionSource.blocks && captionSource.blocks.length > 0) {
                            ...
                            if (relevantBlocks.length > 0) {
                                ...
                                // Wrapped in try-catch: if ASS fails, fall back to basic drawtext.
                                try {
                                    const assContent = generateSubtitleASS({
                                        blocks: localBlocks,
                                    ...
                                    // ---- DRAWTEXT FALLBACK ----
                                    // If ASS generation fails, render basic centered white-on-black drawtext subtitles
                                    // so the clip is still produced (just without fancy karaoke styling).
                                    console.warn(`[REPROCESS] ASS subtitle failed: ${assError.message}. Falling back to drawtext.`);
                                    const { escapeForFFmpegDrawtext: escDrawtext } = require('./utils/text-escaping');
                                    let dtIdx = 0;
                                    const dtFontSize = Math.round(outWidth / 20);
                                    for (const block of localBlocks.slice(0, 50)) { // cap at 50 to avoid filter overflow
                                        const rawText = block.text || (block.words ? block.words.map(w => w.text).join(' ') : '');
                                        if (!rawText.trim()) continue;
                                        const safeText = escDrawtext(rawText);
                                        const st = (block.startTime ?? 0).toFixed(2);
                                        const et = (block.endTime ?? block.startTime ?? 0).toFixed(2);
                                        filterGraph += `;${lastLink}drawtext=text='${safeText}':fontcolor=white:fontsize=${dtFontSize}:x=(w-text_w)/2:y=h*0.85:shadowcolor=black:shadowx=2:shadowy=2:enable='between(t\\,${st}\\,${et})'[v_dt${dtIdx}]`;
                                        lastLink = `[v_dt${dtIdx}]`;
                                        dtIdx++;
                                    }
                                    if (dtIdx > 0) {
                                        console.log(`[REPROCESS] Drawtext fallback: ${dtIdx} subtitle blocks rendered`);
                                    }
                                }
                            } else {
...
const syncAssets = async () => {
    try {
        ...
        if (newCount > 0 || assets.length !== initialCount) {
            await writeAssets(assets);
            console.log(`[ASSET-SYNC] Synced. New: ${newCount}`);
        }
        return assets;
    } catch (err) {
        console.error("[ASSET-SYNC] Error:", err.message);
        return await readAssets();
    }
};
...
app.get('/api/assets', async (req, res) => {
    const freshAssets = await syncAssets();
    ...
    try {
        ...
        if (potentialVtts.length > 0) {
            ...
            try {
                ...
                const newAssets = await readAssets();
...
`;
        ...
        pythonProcess.on('close', (code) => {
            ...
            if (code === 0 && fs.existsSync(jsonPath)) {
                // Update Asset
                (async () => {
                    const updatedAssets = await readAssets();
...
app.post('/api/transcribe/:id', async (req, res) => {
    ...
    (async () => {
        ...
        const saveTranscript = async (text, segments = [], method = 'unknown', lang = 'unknown') => {
            fs.writeFileSync(jsonPath, JSON.stringify({
                text,
                segments,
                language: lang,
        ...
        if (geminiKey && geminiKey.length > 10) {
            ...
            try {
                ...
                if (transcriptText && transcriptText.length > 10) {
                    ...
                    await saveTranscript(transcriptText, segments, 'gemini');
...
app.get('/api/captions/:assetId', async (req, res) => {
...
app.get('/api/captions/clip/:clipFilename', async (req, res) => {
...
// ============================================================
// LICENSE BYPASS ENDPOINTS (ONLYCLICK - unlocked)
// All auth/license API calls return success automatically
// ============================================================
...
app.post('/api/auth/login', (req, res) => {
    ...
    res.json({
        success: true,
        token: 'onlyclick-bypass-token',
        email: email || 'user@onlyclick.app',
        license: { key: 'OC-UNLOCKED', valid: true, plan: 'unlimited' }
    });
});
...
app.get('/api/license/verify', (req, res) => {
    res.json({ valid: true, plan: 'unlimited', status: 'active', email: 'user@onlyclick.app' });
});
...
app.get('/api/license/info', (req, res) => {
    res.json({ valid: true, key: 'OC-UNLOCKED', plan: 'unlimited', status: 'active' });
});

app.post('/api/license/activate', (req, res) => {
    res.json({ success: true, valid: true, plan: 'unlimited', status: 'active' });
});
...
app.get('/api/license/key', (req, res) => {
    res.json({ key: 'OC-UNLOCKED', valid: true });
});
...
app.get('/api/version', (req, res) => {
    res.json({ version: APP_VERSION, name: 'ONLYCLICK' });
});
...
// GET: List all prompts (defaults + user-created)
app.get('/api/prompts', async (req, res) => {
    try {
        const prompts = await readPrompts();
        res.json(prompts);
    } catch (error) {
        console.error('[PROMPTS] List error:', error.message);
        res.status(500).json({ error: error.message });
    }
});

// POST: Create a new custom prompt
app.post('/api/prompts', async (req, res) => {
    try {
        ...
        console.log(`[PROMPTS] Created: "${newPrompt.name}"`);
        res.json(newPrompt);
    } catch (error) {
        console.error('[PROMPTS] Create error:', error.message);
        res.status(500).json({ error: error.message });
    }
});

// PUT: Update a custom prompt
app.put('/api/prompts/:id', async (req, res) => {
    try {
        ...
        if (prompts[idx].isDefault) return res.status(403).json({ error: 'Cannot edit default prompts' });

        if (name) prompts[idx].name = name.trim();
        if (instruction) prompts[idx].instruction = instruction.trim();
        if (icon) prompts[idx].icon = icon;
        prompts[idx].updatedAt = new Date().toISOString();

        await writePrompts(prompts);
        console.log(`[PROMPTS] Updated: "${prompts[idx].name}"`);
        res.json(prompts[idx]);
    } catch (error) {
        console.error('[PROMPTS] Update error:', error.message);
        res.status(500).json({ error: error.message });
    }
});
...
app.delete('/api/prompts/:id', async (req, res) => {
    try {
        ...
        if (prompt.isDefault) return res.status(403).json({ error: 'Cannot delete default prompts' });

        const filtered = prompts.filter(p => p.id !== id);
        writePrompts(filtered);

        console.log(`[PROMPTS] Deleted: "${prompt.name}"`);
        res.json({ success: true });
    } catch (error) {
        console.error('[PROMPTS] Delete error:', error.message);
        res.status(500).json({ error: error.message });
    }
});
...
// GET: Serve clip media for the visual editor (supports range requests)
app.get('/api/editor/clip-media/:projectId/:clipIdx', (req, res) => {
    try {
        const { projectId, clipIdx } = req.params;
        const history = readDb();
        const project = history.find(p => p.id === projectId);
        ...
        // clip.url is a relative URL like /clips/projectId/clip_v123.mp4
        // Convert to absolute file path
        let clipFilePath = clip.url;
        if (clipFilePath.startsWith('/clips/')) {
            clipFilePath = path.join(clipsDirRoot, clipFilePath.replace('/clips/', ''));
        } else if (clipFilePath.startsWith('/')) {
            clipFilePath = path.join(__dirname, '..', clipFilePath);
        }
        ...
        // Set CORP headers for cross-origin iframe access
        res.set('Cross-Origin-Resource-Policy', 'cross-origin');
        res.set('Access-Control-Allow-Origin', '*');
        ...
        if (range) {
            ...
            fs.createReadStream(clipFilePath, { start, end }).pipe(res);
        } else {
            res.writeHead(200, {
        ...
        console.error('[EDITOR] clip-media error:', error.message);
        res.status(500).json({ error: error.message });
    }
});

// POST: Save visual composition from the editor
app.post('/api/editor/save-composition', async (req, res) => {
    try {
        const { projectId, clipIdx, composition } = req.body;
        if (!projectId || clipIdx === undefined || !composition) {
            return res.status(400).json({ error: 'Missing projectId, clipIdx, or composition' });
        }
        ...
        const idx = parseInt(clipIdx);
        if (!history[projectIndex].clips || idx >= history[projectIndex].clips.length) {
            return res.status(404).json({ error: 'Clip not found' });
        }

        // Save composition data on the clip object
        history[projectIndex].clips[idx].visualComposition = composition;
        history[projectIndex].clips[idx].visualCompositionSavedAt = new Date().toISOString();
        writeDb(history);

        console.log(`[EDITOR] Saved visual composition for project=${projectId} clip=${idx}`);
        res.json({
            success: true,
            savedAt: history[projectIndex].clips[idx].visualCompositionSavedAt,
        });
    } catch (error) {
        console.error('[EDITOR] save-composition error:', error.message);
        res.status(500).json({ error: error.message });
    }
});

// GET: Load saved visual composition
app.get('/api/editor/load-composition/:projectId/:clipIdx', (req, res) => {
    try {
        const { projectId, clipIdx } = req.params;
        const history = readDb();
        const project = history.find(p => p.id === projectId);
        ...
        const clip = project.clips[idx];
        if (!clip.visualComposition) {
            return res.status(404).json({ error: 'No visual composition saved for this clip' });
        }

        res.json({
            composition: clip.visualComposition,
            savedAt: clip.visualCompositionSavedAt || null,
        });
    } catch (error) {
        console.error('[EDITOR] load-composition error:', error.message);
        res.status(500).json({ error: error.message });
    }
});
...
const showServerBanner = (usedPort) => {
    ...
    console.log(``);
    console.log(`========================================`);
    console.log(`  ONLYCLICK Server v${APP_VERSION}`);
    console.log(`========================================`);
    ...
    for (let attempt = 0; attempt < MAX_PORT_RETRIES; attempt++) {
        ...
        try {
            ...
            wss.on('connection', (ws) => {
                _origLog('[WS] Client connected to log stream');
                ws.on('close', () => _origLog('[WS] Client disconnected'));
            });

            showServerBanner(actualPort);
            server.timeout = 600000; // 10 minutes (match yt-dlp process timeout)
            return;
        } catch (err) {
            if (err.code !== 'EADDRINUSE' || attempt === MAX_PORT_RETRIES - 1) {
                ...
                if (err.code === 'EADDRINUSE') {
                    ...
                    console.error(`  1. Close other ONLYCLICK instances`);
